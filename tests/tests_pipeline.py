...
def test_15_minute_resolution_integrity():
    ...
    """
    conn = sqlite3.connect(DB_PATH)
    df = pd.read_sql_query("SELECT timestamp FROM day_ahead_prices ORDER BY timestamp DESC LIMIT 10", conn, parse_dates=['timestamp'])
    conn.close()
    ...
    for diff in time_diffs:
        assert diff == pd.Timedelta(minutes=15), f"Data resolution violation: Found {diff} instead of 15 minutes."
...
def test_ui_required_columns_exist():
    ...
    for col in required_columns:
        assert col in df.columns, f"UI Crash Risk: Required column '{col}' is missing from master_features."
...
def test_no_data_leakage_in_target():
    ...
    """
    conn = sqlite3.connect(DB_PATH)
    df = pd.read_sql_query("SELECT price_eur_mwh, target_price_24h_ahead FROM master_features ORDER BY timestamp ASC LIMIT 200", conn)
    conn.close()
    ...
    assert actual_future_price == predicted_target, "Data Leakage Detected: Target variable does not align with the 24-hour future price."
...
def test_safe_insert_duplicate_handling():
    ...
    assert len(df_deduplicated) == 2, "Duplicate handling failed: Script did not drop the overlapping DST timestamp."
