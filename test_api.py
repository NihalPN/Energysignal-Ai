...
try:
    ...
    df = pd.read_sql_query("SELECT timestamp, price_eur_mwh, total_renewable FROM master_features ORDER BY timestamp DESC LIMIT 5", conn)
    ...
    if df.empty:
        ...
        if df['timestamp'].str.contains(today_str).any():
            ...
            print(f"❌ FAILURE: Data stops before {today_str}. The GitHub Action is NOT pulling live data.")
...
try:
    ...
    test_scenario = "Current Market Reality: The price is 75.00 EUR/MWh. Renewable generation is 40000.00 MW."
