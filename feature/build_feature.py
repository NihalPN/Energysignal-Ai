...
def build_and_store_master_features():
    ...
    solar_col = next((col for col in master_df.columns if 'solar' in col.lower() or 'radiation' in col.lower()), None)
    ...
    master_df["is_peak_solar_window"] = ((master_df.index.hour >= 11) & (master_df.index.hour <= 15)).astype(int)
    # -------------------------------------------
