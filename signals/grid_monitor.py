...
def detect_grid_anomalies():
    ...
    gen_df = pd.read_sql_query("SELECT * FROM generation_mix", conn, parse_dates=['timestamp'], index_col='timestamp')
    load_df = pd.read_sql_query("SELECT * FROM actual_load", conn, parse_dates=['timestamp'], index_col='timestamp')
    conn.close()
