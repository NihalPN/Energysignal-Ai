...
def fetch_and_store_weather(start_date: str, end_date: str):
    ...
    try:
        ...
        minutely_15_data = {"timestamp": pd.date_range(
            start=pd.to_datetime(minutely_15.Time(), unit="s", utc=True).tz_convert('Europe/Berlin'),
            end=pd.to_datetime(minutely_15.TimeEnd(), unit="s", utc=True).tz_convert('Europe/Berlin'),
