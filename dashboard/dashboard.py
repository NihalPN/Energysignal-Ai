...
@st.cache_data(
    ttl=3600, show_spinner="🤖 AI Analyst is reading the market..."
...
if df_full.empty:
    st.warning(
        "Database is empty or missing. Please ensure your data pipeline has run and the database is pushed to GitHub."
    )
else:
    # --- MATH & PREDICTIONS ---
    ...
    tab_strategy, tab_preds, tab_live = st.tabs(
        ["Strategy & AI Analyst", "XGBoost 24h Forecasts", "Live Market Monitor"]
    )
    ...
    with tab_strategy:
        ...
        with col2:
            st.metric(
                label="XGBoost 24h Forecast",
                value=f"€{target_price_24h_now:.2f}",
                delta=f"{current_spread:.2f} Spread",
            )
        with col3:
    ...
    with tab_preds:
        ...
        if pred_data:
            ...
            st.dataframe(
                pred_df.style.map(color_profit, subset=["Predicted Price (€/MWh)"]),
    ...
    with tab_live:
        ...
        try:
            ...
            if not today_df.empty:
                ...
                st.dataframe(
                    display_df.style.map(color_profit, subset=["Clearing Price (€/MWh)"]),
                    use_container_width=True,
                    hide_index=True,
                )
            else:
                st.info(
                    f"No prices cleared for today ({today_str}) yet. Ensure the database has been updated."
                )
